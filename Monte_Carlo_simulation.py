...
scatter_matrix_title = 'Monte Carlo Simulation - Scatter Matrix (triangular dists)'
...
plt.title('Monte Carlo Simulation - Distributions (triangular)', fontsize=title_size, fontweight='bold')
max_bin_cnt = 0
for d in range(total_dists):
    h = plt.hist(dists['x' + str(d+1)], bins=histogram_bins_per_dist, label='D' + str(d+1), alpha=0.9, linewidth=0.15)
...
plt.legend(loc='upper left', bbox_to_anchor=(0, 0.995), fontsize=legend_size, facecolor='whitesmoke', framealpha=1).set_zorder(15)
...
plt.grid(True, color='lightgray', linewidth=0.75, alpha=0.5, zorder=0)
ax2 = ax1.twinx()
sns.histplot(dists['x avg'], bins=int(obs_per_dist**(1/3)), alpha=0.8, linewidth=0.15, ax=ax2, label='Count (bin)', zorder=5)
plt.legend(loc='upper left', bbox_to_anchor=(0, 0.95), fontsize=legend_size, facecolor='whitesmoke', framealpha=1).set_zorder(15)
